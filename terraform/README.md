...
| [`cfncompat/`](cfncompat/README.md) | `hashicorp/aws` + `hashicorp/archive` + `cdktn-io/cfncompat` | `aws_s3_bucket` + a per-stack `cfncompat_custom_resource` (no `aws_s3_bucket_notification` anywhere) | same `cfncompat_custom_resource` polyfill |
...
bucket. `cfncompat/` replaces that singleton resource everywhere with a `cfncompat_custom_resource`
driving AWS CDK's own bucket-notifications Lambda handler in its merge ("unmanaged") mode — see
`../docs/OPTIONS.md` (Option A) and `../CONTRACT.md`.
...
`s3.amazonaws.com` invoke permission; `cfncompat/` reuses `aws/`'s copy of this module by relative
path) — see each scenario's own README for apply order and verify-without-credentials steps.
