...
# its own notification target (prefix "b/") via aws_s3_bucket_notification -- the same resource
# stack-a uses for its own target in this scenario, so any replace-vs-merge behavior is not an
# awscc/aws asymmetry, it's aws_s3_bucket_notification's own semantics.
